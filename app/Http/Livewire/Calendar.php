...
class Calendar extends Component
{
    ...
    public function mount(SessionManager $session, string $minDate = null, string $maxDate = null, array $unavailableDate)
    {
    ...
    {
        if(!empty($this->minDate) && Carbon::parse($this->minDate)>session('calendar.now')){
            $this->selectedDay = Carbon::parse($this->minDate)->day;
            $this->selectedMonth = $this->currentMonth = Carbon::parse($this->minDate)->month;
            $this->selectedYear = $this->currentYear = Carbon::parse($this->minDate)->year;
        }else {
    ...
    private function isDayUsed(int $day = null): bool
    {
        $dateDay = Carbon::createFromDate($this->currentYear, $this->currentMonth, $day);
        return in_array($dateDay->format('Y-m-d'), $this->unavailableDate);
    }
    ...
    {
        if (is_null($day) || $this->isDayUsed($day)) {
            return;
        }
        ...
        $this->emit('dateSelected', Carbon::createFromDate($this->currentYear, $this->currentMonth, $day)->format('Y-m-d'));
    }
